...
- Create and edit v1 scene JSON with normal filesystem tools. Use
  `scene_render_file` to validate and render a file before opening it.
- In the Playwright-managed browser, use the clean localhost root and switch from
  Runtime to Editor for visual inspection and human transform adjustments. Scene MCP
  tools provide observation and preview controls; they do not save document edits.
...
Browser pointer input is enabled through `input.canvasPointerEvents` by default. Add `RayInteractable` and react to `Hovered`/`Pressed` for objects that should work with both mouse/touch canvas input and XR rays. XR-specific input lives at `world.input.xr`; use `world.input.keyboard` and `world.input.browserGamepads` for low-level browser controls. Reusable systems should prefer `world.input.actions` for intent such as `locomotion.move` or `locomotion.jump`; opt into browser locomotion bindings with `features.locomotion.browserControls`.
...
#### DON'T use manual Raycaster — use RayInteractable component
...
// ✅ GOOD - add RayInteractable, then query Hovered/Pressed in your system
...
When `.agents/skills/iwsdk-scene-composer/SKILL.md` exists, read it before
composing a static native IWSDK scene from text, a reference image, or both. It
defines the fixed authoring/runtime format, capability-aware editor tool loop,
composition patterns, evidence contract, and editor-to-runtime verification
gate. Claude projects expose the same canonical skill at
...
- For native scene/editor work, call `scene_get_state`. It reports editor, file, runtime, hash, validation, and conflict state and does not require IWER device emulation.
- Before XR device or session actions, call `xr_get_session_status` (or `npx iwsdk xr status`).
- Before starting another server, call `npx iwsdk dev status`. Do not infer server readiness from XR availability.
...
**Before starting a dev server, check `npx iwsdk dev status`.** XR emulation is optional and is not a server-readiness signal.
...
4. For editor work, call `scene_get_state`
5. For XR testing only, call `xr_get_session_status`, then `xr_accept_session`
...
3. **Check runtime entities** — use `ecs_find_entities` to verify live entities exist and find entity indices
