...
Keep the manifest in a dedicated module and configure the Vite plugin to import it:
...
The application imports that same module:
...
await World.create(document.getElementById('scene-container')!, {
  assets,
...
Use public-root URLs for URL assets. Never put URLs or asset declarations in scene
JSON, and never guess a path without checking the project.
...
Panel surfaces are single-sided. Verify the authored rotation from the intended
camera rather than assuming a missing preview is a loader failure. Use
...
- Asset code, scene modules, and the root build without warnings or blocking errors.
