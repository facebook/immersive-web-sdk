...
Completion requires all root/module renders to pass, a clean conflict-free editor
state, nonblank required views, passing required features, and a live application
runtime bound to the expected root/runtime hash. There is no editor publish tool;
record the final evidence and verify the application build normally.
