...
application asset manifest configured through `iwsdkDev({ assetManifest })`.
...
Each imported file must be a valid standalone v1 scene. Imports resolve recursively
in declaration order. Node and prefab IDs are namespaced as
...
World.create(container, { components, level: '/scenes/main.iwsdk.scene.json' });

// vite.config.ts
iwsdkDev({ componentManifest: './src/components.ts' });
```
...
- Do not use non-finite transforms, zero/negative scale, unsafe paths, or unknown
  component fields.
